...
export class Update extends plugin {
  ...
  async runUpdate(isForce) {
    let command = `git -C ./plugins/RocoWorld-plugins/ pull`
    if (isForce) {
      command = `git -C ./plugins/RocoWorld-plugins/ reset --hard origin/main && ${command} --rebase`
      this.e.reply('正在执行强制更新操作，请稍等')
    } else {
      this.e.reply('正在执行更新操作，请稍等')
    }
    /** 获取上次提交的commitId，用于获取日志时判断新增的更新日志 */
    this.oldCommitId = await this.getcommitId('RocoWorld-plugins')
    uping = true
    const ret = await this.execSync(command)
    uping = false

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：RocoWorld-plugins`)
      this.gitErr(ret.error, ret.stdout)
      return false
    }

    /** 获取插件提交的最新时间 */
    const time = await this.getTime('RocoWorld-plugins')

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`RocoWorld-plugins已经是最新版本\n最后更新时间：${time}`)
    } else {
      await this.reply(`RocoWorld-plugins\n最后更新时间：${time}`)
      this.isUp = true
      /** 获取RocoWorld-plugins的更新日志 */
      const log = await this.getLog('RocoWorld-plugins')
